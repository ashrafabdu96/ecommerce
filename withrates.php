...
(SELECT AVG(stars) FROM `reviews` WHERE `reviews`.`item_id_review`=`e_commerce`.`items`.`items_id`) as stars
...
(SELECT `reviews`.`item_id_review`, AVG(`reviews`.`stars`) as avg_rating
FROM `reviews`
